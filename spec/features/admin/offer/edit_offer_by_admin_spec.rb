...
RSpec.describe "Admin can edit an offer", type: :feature do
  ...
  context "a admin user must be able to edit a created offer" do
    ...
    context "when admin has errors of offer data attributes" do
    ...
    context "when admin try save associations" do
      context "when has erros in all associations" do
        scenario "should return an goblal messages error of models associates" do
          ...
          expect(page).to have_css('#age_range', text: 'Desde debe ser igual ó mayor a uno(1)')
          expect(page).to have_css('#age_range', text: 'Hasta debe ser igual ó mayor a uno(1)')
