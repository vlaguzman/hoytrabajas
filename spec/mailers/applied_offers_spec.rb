...
RSpec.describe AppliedOffersMailer, type: :mailer do
  ...
  let(:mail_data) { {offer_title: offer.title, user_last_name: "#{user.name} #{user.last_name}" , company_name: company.name, user_email: user.email, company_email: company.email } }
  ...
  describe "company_notification" do
    ...
    it "render the body" do
      ...
      expect(email.body.encoded).to match(/Han aplicado a tu oferta/)
