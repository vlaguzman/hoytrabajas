...
RSpec.describe "users/candidates/step_two" do
  it "Should render users candidate step_two template" do
    ...
    #form
    expect(rendered).to have_tag(:form, with: { id: "candidate_step_two" }) do
      with_tag(:input, with: { name: "user[about_me]", type: "text" })
      with_select("user[sex]")
      with_tag(:input, with: { name: "user[birthday]", type: "date" })
      with_select("user[limitations][]")
      with_select("user[educational_degrees][]")
